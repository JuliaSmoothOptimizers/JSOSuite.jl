...
"""
    quadprog(Q, g; x0, Aineq, blow, bupp, Aeq, beq, lvar, uvar, kwargs...)
...
function quadprog(
  ...
  Aineq::MatrixOrOperator = zeros(0, length(g)),
  blow::AbstractVector = fill(-Inf, size(Aineq, 1)),
  bupp::AbstractVector = fill(Inf, size(Aineq, 1)),
  lvar::AbstractVector = fill(-Inf, length(g)),
  uvar::AbstractVector = fill(Inf, length(g)),
  Aeq::MatrixOrOperator = zeros(0, length(g)),
  ...
  output = ipopt(nlp, print_level = 0; kwargs...)
  return output
end

"""
    quadprog(Q, g, A, b; x0, kwargs...)
    quadprog(Q, g; Aeq = A, beq = b, x0 = x0, kwargs...)
...
"""
    quadprog(Q, g, A, b, Aeq, beq; x0, kwargs...)
...
"""
    quadprog(Q, g, A, b, lvar, uvar; x0, kwargs...)
...
"""
    quadprog(Q, g, A, b, Aeq, beq, lvar, uvar; x0, kwargs...)
...
"""
    quadprog(Q, g, Aineq, blow, bupp; x0, kwargs...)
